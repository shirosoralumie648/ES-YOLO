...
# Modified by Diego Augusto Silva to include AutoBackend for architectures with memory cells
...
class AutoBackend(nn.Module):
    ...
    def warmup(self, imgsz=(1, 3, 640, 640)):
        ...
        if any(warmup_types) and (self.device.type != 'cpu' or self.triton):
            print("this is the image size", imgsz)
...
class AutoBackendMemory(AutoBackend):

    def forward(self, im, hidden_states, augment=False, visualize=False):
        """
        ...
        if self.fp16 and im.dtype != torch.float16:
            im = im.half()  # to FP16
        if self.pt or self.nn_module:  # PyTorch
            y = self.model(im, hidden_states = hidden_states, augment=augment, visualize=visualize) if augment or visualize else self.model(im, hidden_states)
    ...
    def warmup(self, imgsz=(1, 1, 320, 320)):
        """
        Warm up the model by running one forward pass with a dummy input.
        ...
        if any(warmup_types) and (self.device.type != 'cpu' or self.triton):
            ...
            if isinstance(imgsz[2],list):
               imgsz = (imgsz[0], imgsz[1], imgsz[2][0], imgsz[3][0])

            im = torch.empty(*imgsz, dtype=torch.half if self.fp16 else torch.float, device=self.device)  # input

            for _ in range(2 if self.jit else 1):  #
                self.forward(im, hidden_states)  # warmup
