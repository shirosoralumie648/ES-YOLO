...
from ultralytics.nn.modules import (C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x, Classify,
                                    ...
                                    GhostBottleneck, GhostConv, Segment,Conv_LSTM)
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if verbose:
            self.info(imgsz = imgsz)
            #print("this is the info:", self.info())
            LOGGER.info('')
...
class DetectionModel2(BaseModel):
    # YOLOv8 detection model
    def __init__(self, cfg='yolov8n.yaml', imgsz = 320, ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        super().__init__()
        ...
        if isinstance(m, (Detect, Segment)):
            ...
            s = 256  # 2x min stride
            m.inplace = self.inplace
            #m.stride = torch.tensor([s / x.shape[-2] for x,y in self.forward(torch.zeros(1, ch, s, s))])  # forward
            x, _ = self.forward(torch.zeros(1, ch, s, s))
            m.stride = torch.tensor([s / x[i].shape[-2] for i in range(len(x))])
            self.stride = m.stride
            m.bias_init()  # only run once
        # Init weights, biases
        initialize_weights(self)
    ...
    def forward(self, x, hidden_states = {"0": None, "1": None, "2": None, "3": None},augment=False, profile=False, visualize=False):

        return self._forward_once(x, hidden_states, profile, visualize)  # single-scale inference, train
    ...
    def _forward_once(self, x, hidden_states, profile=False, visualize=False):
        """
        Perform a forward pass through the network.
        ...
        for m in self.model:
            ...
            if profile:
                ...
                self._profile_one_layer(m, x,dt)
            if m.type in {"ultralytics.nn.modules.Conv_LSTM"}:
             
             hidden_states[str(self.lstm_layer)] = m(x,hidden_states[str(self.lstm_layer)])
    ...
    def _profile_one_layer(self, m, x, dt):
        ...
        if m.type in {"ultralytics.nn.modules.Conv_LSTM"}:
            n1, c1, h1, w1 = x.shape
            cur_hid = [torch.rand(n1,c1,h1,w1).to(x), torch.rand(n1,c1,h1,w1).to(x)]
            o = thop.profile(m, inputs=(x.clone() if c else x, cur_hid), verbose=False)[0] / 1E9 * 2 if thop else 0  # FLOPs
        else:
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    nc, gd, gw, act = d['nc'], d['depth_multiple'], d['width_multiple'], d['act']
    if act:
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                ...
                BottleneckCSP, C1, C2, C2f,C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x,Conv_LSTM}:
            c1, c2 = ch[f], args[0]
