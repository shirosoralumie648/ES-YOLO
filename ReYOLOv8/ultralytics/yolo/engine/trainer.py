...
class BaseTrainer:
    ...
    def _setup_ddp(self, rank, world_size):
        ...
        dist.init_process_group('nccl' if dist.is_nccl_available() else 'gloo', timeout=timedelta(seconds=3600),rank=rank, world_size=world_size)
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    #print("Data types currently under use, batch: ", batch["img"].dtype, " preds: " , preds[0].dtype)
                    #time.sleep(1)
    ...
    def save_model(self):
        ...
        print(self.best_fitness, self.fitness)    
    ...
    def build_optimizer(model, name='Adam', lr=0.001, momentum=0.9, decay=1e-5):
        ...
        for v in model.modules():
            ...
            if hasattr(v, 'bias') and isinstance(v.bias, nn.Parameter):  # bias (no decay)
                g[2].append(v.bias)
            if hasattr(v, 's') and isinstance(v.s,nn.Parameter):  # weight (with decay)
                
                g[0].append(v.s)
            if hasattr(v, 'alpha') and isinstance(v.alpha,nn.Parameter):  # bias (no decay)
